...
export default function PlayerPool({ players, setPlayers }: PlayerPoolProps) {
  ...
  const updatePlayerPool = (updater: (players: Player[]) => Player[]) => {
  ...
  useEffect(() => {
    ...
    if (saved && lastUpdated === today) {
      try {
        ...
        setPlayers(parsed); // ✅ No filtering based on current state
      } catch (e) {
  ...
  const handleRemoveGuest = (guestId: string) => {
    ...
    if (!isGuest) return;
    setPlayers((prev) => prev.filter((p) => (p._id || p.id) !== guestId));
  };
...
function SortablePoolPlayer({
  ...
  const {
    ...
    transition,
    isDragging
  } = useSortable({ id: `pool-${player._id || player.id}`, data: { from: 'pool', player, index } });
  ...
  const isGuest = (player._id || player.id)?.toString().startsWith('guest');
  ...
  const isNewlyAdded = justAddedId === player.id;
  ...
  return (
    ...
    >
      {isGuest && (
        <button
          onClick={() => handleRemoveGuest?.(player._id || player.id)}
        ...
        >
          ...
          >
            <div className="space-y-2">
              {players.map((player, index) => (
                <SortablePoolPlayer
                  key={`pool-${player._id || player._id}`}
                  player={player}
                  index={index}
                  justAddedId={justAddedPlayerId ?? undefined}
                  handleRemoveGuest={handleRemoveGuest}
...
function SortablePoolPlayer({
  ...
  const isGuest = player._id?.startsWith('guest');
  const genderColor = isGuest
    ? player.gender === 'Female'
      ? 'bg-pink-500'
      : 'bg-blue-500'
  ...
  return (
    ...
    >
      {isGuest && (
        <button
          onClick={() => handleRemoveGuest?.(player._id)}
          className="absolute top-1 right-1 text-white bg-red-500 hover:bg-red-600 rounded-full p-[2px] text-xs"
          title="Remove Guest"
        >
          ×
